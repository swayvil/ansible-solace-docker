...
# save broker inventory
#  ansible-playbook -i "./broker.inventory.yml" "$scriptDir/service.playbook.yml" --extra-vars "WORKING_DIR=$WORKING_DIR"
#  code=$?; if [[ $code != 0 ]]; then echo ">>> XT_ERROR - $code"; exit 1; fi

# configure broker service
  ansible-playbook -i "$scriptDir/broker.inventory.yml" "$scriptDir/configure.playbook.yml"  --extra-vars "AUTO_RUN=$AUTO_RUN"
  code=$?; if [[ $code != 0 ]]; then echo ">>> XT_ERROR - $code"; exit 1; fi
